...
namespace XYJHZX_MVC.Lib
{
    ...
    {
        const string Str_DataKey = "fsd2rmyy";
        private IXmlOper IXmlConfig = new XmlOper();//Xml设置接口
        private string str_OraType = "ORACLE126";
        private OracleConnection _OraCon = new OracleConnection();
        ...
        public bool ConInit(out string str_msg)
        {
            try
            {
                ...
                if (IXmlConfig.GetDataForXml(out str_msg, ref arr_strConfig, "ORACLE126"))
                {
                    if (arr_strConfig.Count > 1 && !string.IsNullOrEmpty(arr_strConfig[1]))
                    {
                        ServerIP = Crypt.DesDecrypt(Str_DataKey, arr_strConfig[1].Trim());
                    }
                    ...
                    if (arr_strConfig.Count > 0 && !string.IsNullOrEmpty(arr_strConfig[0]))
                    {
                        userid = Crypt.DesDecrypt(Str_DataKey, arr_strConfig[2].Trim());
                    }
                    if (arr_strConfig.Count > 0 && !string.IsNullOrEmpty(arr_strConfig[0]))
                    {
                        userpwd = Crypt.DesDecrypt(Str_DataKey, arr_strConfig[3].Trim());
                    }
                    if (arr_strConfig.Count > 0 && !string.IsNullOrEmpty(arr_strConfig[0]))
                    {
                        DatabaseName = Crypt.DesDecrypt(Str_DataKey, arr_strConfig[4].Trim());
                    }
                    string strCon = "Data Source=(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=" + ServerIP +
                       ")(PORT=1521" + "))(CONNECT_DATA=(SERVICE_NAME=" + DatabaseName + ")))" +
                       ";User ID=" + userid +
                       ";Password=" + userpwd + ";";
                    _OraCon = new OracleConnection(strCon);
                    return true;
                }
                else
                {
        ...
        {
            string str_sql = string.Format(@"select SplitName from t_bse_DateTimeSplit Where Status = 1 And BegintTime < '{0}' And EndTime >= '{0}' order by SeqId", str_Time);
            _ResultData = new DataSet();
            return SelectOpr(out str_msg, str_sql, ref _ResultData);
        }
        ...
        {
            string str_sql = "select nvl(max(signinseq),0) seq from V_CurrentSchedul";
            if (arr_condition.Count() > 0 && !string.IsNullOrEmpty(arr_condition[0]) && arr_condition[0].ProcessSqlStr())
            {
        ...
        {
            string str_sql = "select nvl(max(signinseq),0) seq from V_SchedulMain";
        ...
        private bool SelectOpr(out string str_msg, string str_sql, ref DataSet _resultData)
        {
            ...
            {
                _OraCon.Open();
                OracleDataAdapter _adp = new OracleDataAdapter(str_sql, _OraCon);
        ...
        private bool ExecOpr(out string str_msg, List<string> arr_strSql)
        {
            ...
            {
                _OraCon.Open();
                OracleCommand _sqlCmd = _OraCon.CreateCommand();
                string str_cmd = "";
                str_msg = "";
